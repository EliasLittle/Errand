...
fn compile_expression(ctx: &mut PreIR, expr: &Expression) -> Result<instr_index, String> {
    match expr {
        ...
        Expression::Return(expr) => {
            ...
            let instr = Instr::Return(ReturnData {
                value: value_idx,
            });
            Ok(ctx.emit_instruction(instr))
        }
...
mod tests {
    use super::*;
    use crate::frontend::ast::{Program, Expression, Id, BinaryOperator, Parameter, TypeExpression};
    ...
    fn test_basic_ir_generation() {
        ...
        assert!(matches!(preir.instructions[0], Instr::Literal(LiteralPl::Int(42))));
    }
        ...
        let program = Program {
            expressions: vec![
                Expression::FunctionDefinition {
                    id: Id { name: "add".to_string() },
                    parameters: vec![
                        Parameter { id: Id { name: "a".to_string() }, type_expr: Some(TypeExpression::Int) },
                        Parameter { id: Id { name: "b".to_string() }, type_expr: Some(TypeExpression::Int) },
                    ],
                    body: Box::new(Expression::Return(Some(Box::new(Expression::BinaryOp {
                        operator: BinaryOperator::Add,
                        left: Box::new(Expression::Identifier {
                            id: Id { name: "a".to_string() },
                            type_expr: None,
                        }),
                        right: Box::new(Expression::Identifier {
                            id: Id { name: "b".to_string() },
                            type_expr: None,
                        }),
                    })))),
