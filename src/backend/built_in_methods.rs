...
pub fn emit_new(
    ...
    malloc_func: Option<(
        ...
        &mut cranelift_object::ObjectModule,
    ...
    let struct_ptr = if struct_size <= 128 && !is_non_leaf {
        ...
        // Heap allocate
        let (malloc_id, module, func) =
            malloc_func.expect("malloc function required for heap allocation");
    ...
    for (i, field) in struct_info.fields.iter().enumerate() {
        ...
        let store_value = if builder.func.dfg.value_type(value) != cranelift_type {
            ...
            match (src_type, cranelift_type) {
                // Integer widening
                (t1, t2) if t1.is_int() && t2.is_int() && t2.bits() > t1.bits() => {
                    builder.ins().uextend(cranelift_type, value)
                }
                // Integer narrowing
                (t1, t2) if t1.is_int() && t2.is_int() && t2.bits() < t1.bits() => {
                    builder.ins().ireduce(cranelift_type, value)
                }
                // Float widening
                (t1, t2) if t1.is_float() && t2.is_float() && t2.bits() > t1.bits() => {
                    builder.ins().fpromote(cranelift_type, value)
                }
                // Float narrowing
                (t1, t2) if t1.is_float() && t2.is_float() && t2.bits() < t1.bits() => {
                    builder.ins().fdemote(cranelift_type, value)
                }
        ...
        trace!(
            "Storing field '{}' at struct_ptr: {:?}, offset: {}",
            field.name,
            struct_ptr,
            offset
        );
...
pub fn emit_mem_load(builder: &mut FunctionBuilder, ptr: Value, offset: Value) -> Value {
    let ptr_with_offset = builder.ins().iadd(ptr, offset);
    builder
        .ins()
        .load(types::I64, MemFlags::new(), ptr_with_offset, 0)
}

pub fn emit_mem_store(builder: &mut FunctionBuilder, ptr: Value, offset: Value, value: Value) {
    let ptr_with_offset = builder.ins().iadd(ptr, offset);
    builder
        .ins()
        .store(MemFlags::new(), value, ptr_with_offset, 0);
}
...
pub fn emit_getfield(
    ...
    let field = struct_info
        ...
        .ok_or_else(|| {
            format!(
                ...
                field_symbol, struct_type
            )
        })?;
    let offset = field.offset as i32;
    trace!("Pointer: {:?}", struct_ptr);
    trace!("Offset: {:?}", offset);
    let cranelift_type = match &field.ty {
        crate::backend::structs::Type::Int => types::I64, // or types::I32 if your Int is 32-bit
        crate::backend::structs::Type::Float => types::F64, // or types::F32 if your Float is 32-bit
    ...
    Ok(builder
        .ins()
        .load(cranelift_type, MemFlags::new(), struct_ptr, offset))
}
